...
from market_anomaly_detection.anomaly_models.statistical_models import ZScoreDetector, IQRDetector
from market_anomaly_detection.anomaly_models.machine_learning_models import (
    IsolationForestDetector,
    LOFDetector,
)
from market_anomaly_detection.anomaly_models.ensemble_model import EnsembleAnomalyDetector
from market_anomaly_detection.post_analysis.statistical_analyzers import (
    PCAAnalyzer,
    CorrelationAnalyzer,
    AnomalyStatisticsAnalyzer,
)
from market_anomaly_detection.visualization.plotter import AnomalyVisualizer
from market_anomaly_detection.base.interfaces import BasePipeline
...
class MarketAnomalyDetectionPipeline(BasePipeline):
    ...
    def _initialize_detectors(self) -> list:
        ...
        return [
            ...
            LOFDetector(n_neighbors=config.lof_neighbors, contamination=config.contamination),
        ]
    ...
    def run(self, data: pd.DataFrame = None) -> Dict[str, Any]:
        ...
        # Calculate Z-scores and IQR flags on the full dataset (mimicking Notebook 1.5)
        window_z = self.config.features.window_z
        window_iqr = self.config.features.window_iqr

        for col in valid_cols:
            # Z-scores
            mu = self.df_featured[col].rolling(window_z).mean()
            sd = self.df_featured[col].rolling(window_z).std()
            self.df_featured[f"{col}_z"] = (self.df_featured[col] - mu) / (sd + 1e-8)

            # IQR flags
            q1 = self.df_featured[col].rolling(window_iqr).quantile(0.25)
            q3 = self.df_featured[col].rolling(window_iqr).quantile(0.75)
            iqr = q3 - q1
            self.df_featured[f"{col}_iqr_flag"] = (
                (self.df_featured[col] < (q1 - 1.5 * iqr))
                | (self.df_featured[col] > (q3 + 1.5 * iqr))
            ).astype(int)

        # Precompute master flags
        if "log_return_z" in self.df_featured.columns:
            self.df_featured["precomputed_z_score"] = self.df_featured["log_return_z"]
        else:
            z_cols = [f"{c}_z" for c in valid_cols if f"{c}_z" in self.df_featured.columns]
            self.df_featured["precomputed_z_score"] = self.df_featured[z_cols].abs().max(axis=1)

        self.df_featured["precomputed_z_flag"] = (
            self.df_featured["precomputed_z_score"].abs() >= 3
        ).astype(int)

        iqr_cols = [
            f"{c}_iqr_flag" for c in valid_cols if f"{c}_iqr_flag" in self.df_featured.columns
        ]
        self.df_featured["precomputed_iqr_flag"] = (
            self.df_featured[iqr_cols].any(axis=1).astype(int)
        )

        ml_cols = []
        for col in valid_cols:
            ml_cols.append(col)
            if f"{col}_z" in self.df_featured.columns:
                ml_cols.append(f"{col}_z")

        # Include precomputed columns in dropna so they are preserved
        keep_cols = ml_cols + ["precomputed_z_flag", "precomputed_iqr_flag", "precomputed_z_score"]
        self.df_model = self.df_featured.dropna(subset=keep_cols).copy().reset_index(drop=True)

        # Step 5: Anomaly detection
        self.logger.info("[Step 5] Running anomaly detectors...")
        X = self.df_model[ml_cols].values
        ...
        for detector in self.detectors:
            if detector.name == "ZScoreDetector":
                detector.precomputed_flags = self.df_model["precomputed_z_flag"].values
                detector.precomputed_scores = self.df_model["precomputed_z_score"].values
            elif detector.name == "IQRDetector":
                detector.precomputed_flags = self.df_model["precomputed_iqr_flag"].values
                detector.precomputed_scores = np.zeros(len(self.df_model))
        ...
        self.df_model["ml_ensemble_flag"] = getattr(
            self.ensemble_detector, "ml_ensemble_flags_", np.zeros_like(ensemble_flags)
        )
        ...
        corr_features = valid_cols + ["pca1", "pca2"]
        corr_cols = [c for c in corr_features if c in self.df_model.columns]
        corr_results = self.correlation_analyzer.analyze(self.df_model, corr_cols)
        ...
        detailed_stats = {}
        cols_to_check = {
            "IQR": "IQRDetector_flag",
            "Z-Score": "ZScoreDetector_flag",
            "Isolation Forest": "IsolationForestDetector_flag",
            "LOF": "LOFDetector_flag",
            "ML Ensemble": "ml_ensemble_flag",
            "Combined": "is_anomaly",
        }
        for name, col in cols_to_check.items():
            if col in self.df_model.columns:
                detailed_stats[name] = int(self.df_model[col].sum())
        stats_results["detailed_counts"] = detailed_stats
        ...
        self.results["visualizations"] = {
            ...
            "correlation_heatmap": self.visualizer.plot_correlation_heatmap(
                ...
                title="Correlation Matrix (Features + PCA Components)",
            ),
        }
    ...
    def save_artifacts(self, output_dir: str | Path = "models") -> Path:
        ...
        if self.scaler is not None:
            joblib.dump(self.scaler, output_dir / "data_scaler.joblib")

        # Save PCA artifacts
        if self.pca_analyzer.pca is not None:
            joblib.dump(self.pca_analyzer.pca, output_dir / "pca_model.joblib")
        ...
        for detector in self.detectors:
            ...
            if model is None:
                continue
            base_name = name_map.get(detector.name, detector.name.replace("Detector", "").lower())
            joblib.dump(model, output_dir / f"{base_name}.joblib")

        # Save ensemble detector
        if getattr(self, "ensemble_detector", None) is not None:
            joblib.dump(self.ensemble_detector, output_dir / "combined_ensemble_model.joblib")
...
def run_pipeline(
    csv_path: str = None, config: PipelineConfig = None
