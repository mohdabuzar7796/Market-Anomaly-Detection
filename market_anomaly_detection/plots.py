...
def _metadata_path() -> Path:
    """Return the path for plot metadata storage."""
    return _ensure_plots_dir() / 'plots_metadata.json'
...
def save_plot(fig, plot_id, prefix=''):
    ...
    plot_file = _ensure_plots_dir() / filename
...
def list_saved_plots():
    ...
    html_files = sorted(_ensure_plots_dir().glob('*.html'))
    if not html_files:
